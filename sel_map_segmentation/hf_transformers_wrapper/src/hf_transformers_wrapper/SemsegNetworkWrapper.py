...
class SemsegNetworkWrapper():
    ...
    def __init__(self, model:str = '', args:dict = {}):
        ...
        # self.config = Mask2FormerConfig(**args)

        # setup the network model
        self.processor = AutoImageProcessor.from_pretrained(model)
        self.model = Mask2FormerForUniversalSegmentation.from_pretrained(model).to(self.device)
    ...
    def runSegmentation(self, pil_image:Image, return_numpy:bool=True, one_hot:bool=False):
        ...
        with torch.no_grad():
            ...
            classes = outputs.class_queries_logits.softmax(dim=-1)[..., :-1]
            probs = masks.sigmoid()

            segmentation = torch.einsum("bqc, bqhw -> bchw", classes, probs)

            scores = torch.nn.functional.interpolate(
                segmentation, size=(height, width), mode="bilinear", align_corners=False
            )[0]
        ...
        # scores = self.processor.post_process_semantic_segmentation(
